...
import type { IPassageiroForm } from '@/types/passageiro.types';
...
const formState: IPassageiroForm = reactive({
  nome: '',
  email: '',
  nascimento: null,
  telefone: '',
...
onMounted(() => {
  if (window.PASSAGEIRO_DATA) {
    const dadosIniciais = window.PASSAGEIRO_DATA;
...
<template>
  <div class="passageiro-create">
    <form @submit.prevent="handleSubmit">
      ...
      <div class="mt-4">
        <Button label="Salvar alterações" class="passageiro-create__data" icon="pi pi-plus" type="submit" />
      </div>
    </form>
  </div>
</template>
